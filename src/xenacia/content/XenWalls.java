...
public class XenWalls{
    ...
    public static void load(){
        ...
        bulkTitaniumAlloyWall = new Wall("bulk-titanium-alloy-wall"){{
            requirements(Category.defense, with(Items.titanium, 8 * 4, Items.graphite, 4 * 4, XenItems.iron, 6 * 4));
            health = 1050 * 2 * 4;
            size = 2;
        ...
        bulkTitaniumAlloyWallLarge = new Wall("bulk-titanium-alloy-wall-large"){{
            requirements(Category.defense, with(Items.titanium, 8 * 9, Items.graphite, 4 * 9, XenItems.iron, 6 * 9));
            health = 1050 * 2 * 4;
            size = 3;
        ...
        bulkTitaniumAlloyWallHuge = new Wall("bulk-titanium-alloy-wall-huge"){{
            requirements(Category.defense, with(Items.titanium, 8 * 16, Items.graphite, 4 * 16, XenItems.iron, 6 * 16));
            health = 1050 * 2 * 4;
            size = 4;
        ...
        bulkCobaltWall = new Wall("bulk-cobalt-wall"){{
            requirements(Category.defense, with(XenItems.cobalt, 12 * 4, Items.titanium, 6 * 4));
            health = 1300 * 2 * 4;
            size = 2;
            researchCost = with(XenItems.cobalt, 2500, Items.titanium, 1500);
        }};
        bulkCobaltWallLarge = new Wall("bulk-cobalt-wall-large"){{
            requirements(Category.defense, with(XenItems.cobalt, 12 * 9, Items.titanium, 6 * 9));
            health = 1300 * 2 * 9;
            size = 3;
            researchCost = with(XenItems.cobalt, 3000, Items.titanium, 2000);
        }};
        bulkCobaltWallHuge = new Wall("bulk-cobalt-wall-huge"){{
            requirements(Category.defense, with(XenItems.cobalt, 12 * 16, Items.titanium, 6 * 16));
            health = 1300 * 2 * 16;
            size = 4;
